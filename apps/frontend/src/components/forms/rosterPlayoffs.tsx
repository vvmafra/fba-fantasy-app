...
export function RosterPlayoffsForm({ isOpen, onClose, teamId, seasonId, isAdmin, onSuccess, deadline, hasExistingRoster }: RosterPlayoffsFormProps) {
  ...
  const loadTeamData = async () => {
    setLoading(true);
    try {
      // Verificar se já existe um roster playoffs para este time na temporada
      const existingRoster = await rosterPlayoffsService.getRosterByTeamAndSeason(teamId, seasonId);
      if (existingRoster) {
        ...
        toast({
          title: "Roster Playoffs Carregado",
          description: "Roster playoffs existente carregado. Você pode fazer alterações e salvar.",
        });
      }
          ...
          const playersFromRoster = orderedPlayerIds
            .map(playerId => {
              ...
              return {
                ...
                isGLeague: false, // Playoffs não têm G-League
  ...
  const handlePlayerClick = (playerId: number) => {
    ...
    setStarters(newStarters);
    setBench(newBench);
    // debouncedSave(newStarters, newBench); // Assuming debouncedSave is defined elsewhere
    setSelectedPlayerId(null);
  };

  // Usar hook de validação
  const validationErrors = useRosterPlayoffsValidation(players, starters, bench, rotationStyle, gameStyle, offenseStyle, defenseStyle);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (existingRosterId) {
        ...
        await rosterPlayoffsService.updateRoster(existingRosterId, rosterData);
        
        toast({
          title: "Sucesso",
          description: "Roster playoffs atualizado com sucesso!",
        });
      } else {
        // Criar novo roster
        await rosterPlayoffsService.createRoster(rosterData);
        
        toast({
          title: "Sucesso",
          description: "Roster playoffs criado com sucesso!",
        });
      }
      ...
      toast({
        title: "Erro",
        description: "Erro ao criar roster playoffs",
        variant: "destructive"
      });
    } finally {
      setSubmitting(false);
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">
                O prazo para envio do roster playoffs já passou. Aguarde a próxima temporada para um novo ciclo.
              </p>
            </div>
            <DialogFooter>
      ...
      <DialogContent className="max-w-4xl w-full max-h-[90vh] overflow-y-auto p-4 md:p-6">
        ...
        <DialogFooter className="flex flex-col md:flex-row gap-2 md:gap-3">
          ...
          <Button 
            ...
            className="w-full md:w-auto"
          >
            {submitting ? 'Salvando...' : (deadline && new Date().getTime() > deadline.getTime()) ? 'Prazo Expirado' : 'Salvar Roster Playoffs'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
